...
function MaiorMenorValor(listaVendas) {

    let lista = listaVendas.flat();

    let listaFiltrada = [];

    for (let numero of lista) {
        if (numero >= 20 && numero <= 500) {
            listaFiltrada.push(numero);
        }
    }

    if (listaFiltrada.length === 0) {
        return 'Nenhuma venda valida';
    } else {
        const ordemCrescente = (a, b) => a - b;

        let listaCrescente = listaFiltrada.sort(ordemCrescente);

        let qtdItens = listaCrescente.length;

        let maior = listaCrescente[qtdItens - 1];

        let menor = listaCrescente[0];

        return [menor, maior];
    }
}

console.log(MaiorMenorValor([[5, 600], [430, 410], [11], []]));